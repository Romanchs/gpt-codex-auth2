...
export const FormTKO = ({ disputeEntity }) => {
  ...
  return (
    <>
      <Grid container alignItems={'flex-start'} spacing={3}>
        ...
        {checkPermissions('DISPUTES.TKO_ENTITY.FIELDS.AKO', DISPUTE_AKO_ALLOWED_ROLES) && disputeEntity?.ako && (
        ...
        <Grid item xs={12} md={4} lg={3}>
          <StyledInput label={t('FIELDS.NUMBER_OF_CONFLICTING_ATTRIBUTES')} value={disputeEntity?.properties_count} readOnly />
        </Grid>

        {disputeEntity?.tko_range && (
          <Grid item xs={12} md={4} lg={3}>
            <StyledInput
              label={t('FIELDS.TKO_RANGE')}
              value={disputeEntity?.tko_range ? disputeEntity.tko_range : '24.09.2022 00:00 - 25.09.2022 00:00'}
              readOnly
            />
          </Grid>
        )}
