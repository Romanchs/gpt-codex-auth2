...
const globalLock = [
  ...
  { id: 'interval_type2', name: 'interval_type', label: 'FIELDS.PERIOD', defaultOptions: defaultOptions.interval_type },
  {
    id: 'interval_divider2',
    name: 'interval_divider',
    label: 'FIELDS.DAY_NUMBER',
    defaultOptions: defaultOptions.interval_divider
  },
  { id: 'interval_time2', name: 'interval_time', label: 'FIELDS.TIME', md: 2 },
  { id: 'active2', name: 'active', md: 6, textAlign: 'right' },
  { id: 'interval_type3', name: 'interval_type', label: 'FIELDS.PERIOD', defaultOptions: defaultOptions.interval_type },
  {
    id: 'interval_divider3',
    name: 'interval_divider',
    label: 'FIELDS.DAY_NUMBER',
    defaultOptions: defaultOptions.interval_divider
  },
  { id: 'interval_time3', name: 'interval_time', label: 'FIELDS.TIME', md: 2 },
  { id: 'active3', name: 'active', md: 6, textAlign: 'right' }
];
...
const ZTab = () => {
  ...
  const { data: settings } = useInitPMZQuery();
  const { data: points, isFetching } = useGetListPMZQuery(params);
  const [updateLockGlobalZ, { error }] = useUpdateLockGlobalPMZMutation({ fixedCacheKey: 'PMZ_updateLock' });
  const [updatePoints] = useUpdatePointsPMZMutation({ fixedCacheKey: 'PMZ_updatePoints' });
  ...
  const handleChangeToggle = async (id, value, index) => {
    ...
    const { error } = await updateLockGlobalZ({ body, action: value ? 'lock' : 'unlock' });
    if (!error) {
      changeSettings({ [id]: value });
      dispatch(mainApi.util.invalidateTags([TAGS.PROCESS_MANAGER_Z_SETTINGS]));
    }
  };
