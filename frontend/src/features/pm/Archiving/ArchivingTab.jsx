...
import { usePmArchivingTSQuery } from './api';
import Row from './Row';
import ArchivingAutocompleteFormField from './ArchivingAutocompleteFormField';
...
const ArchivingTab = () => {
  ...
  const FORM_NAME = 'archiving-sub_filters';
  ...
  const { currentData, isFetching } = usePmArchivingTSQuery(params);
  ...
  const getSearch = (id, type, minWidth, disabled, defaultValue) => {
    switch (type) {
      case 'select':
        return (
          <TableSelect
            value={defaultValue || search[id]}
      ...
      default:
        return (
          <SearchField
            name={id}
            onSearch={(i) => onSearch(id, i[id], 'text')}
            minSymbols={id === 'process_id' ? 1 : 3}
          />
        );
    }
  };
  ...
  return (
    <>
      <StyledTable spacing={0}>
        <StickyTableHead>
          <TableRow>
            {columns.map(({ id, label, type, minWidth, width, disabled, defaultValue }) => (
              <TableHeadCell sx={{ minWidth, width }} key={id} title={t(label)}>
                {getSearch(id, type, minWidth, disabled, defaultValue)}
              </TableHeadCell>
            ))}
