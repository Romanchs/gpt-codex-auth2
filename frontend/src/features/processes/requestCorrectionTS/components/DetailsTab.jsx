...
const DetailsTab = ({ error }) => {
  const dispatch = useDispatch();
  const { t } = useTranslation();
  const data = useSelector((store) => store.requestCorrectionTS.data);
  const [userSettedDateOfLockTs, setUserSettedDateOfLockTs] = useState(false);
  ...
  const IS_EDITABLE = currentData?.can_done;
  ...
  useEffect(() => {
    if (currentData?.started_at && IS_EDITABLE && !userSettedDateOfLockTs) {
      dispatch(setData({ ...data, date_of_lock_ts: moment(currentData?.started_at).add(2, 'hours') }));
    }
  }, [currentData, userSettedDateOfLockTs]);
  ...
  const handleOnChange = (key) => (value) => {
    if (key === 'comment') value = value.target.value;
    if (key === 'date_of_lock_ts') setUserSettedDateOfLockTs(true);
    ...
    <>
      <Box className={'boxShadow'} sx={{ mt: 2, mb: 2, p: 3 }}>
        <Grid container spacing={2} alignItems={'flex-start'}>
          ...
          <Grid item xs={12}>
            <StyledInput
              label={t('FIELDS.CORRECTION_DESCRIPTION')}
      ...
      <Box className={'boxShadow'} sx={{ mt: 2, mb: 2, p: 3 }}>
        <Grid container spacing={2} alignItems={'flex-start'}>
          ...
          <Grid item xs={12} md={6} lg={3}>
            {IS_EDITABLE ? (
              <DateTimePicker
                ...
                value={data.date_of_lock_ts}
                onChange={handleOnChange('date_of_lock_ts')}
                error={error?.data?.date_of_lock_ts}
              />
            ) : (
              <StyledInput
                label={t('FIELDS.CLOSE_GATE_DATETIME')}
          ...
          <Grid item xs={12} md={6} lg={3}>
            <StyledInput
              label={t('FIELDS.FACT_CLOSE_GATE_DATETIME')}
              value={
                currentData?.additional_data?.fact_date_of_lock_ts &&
                moment(currentData?.additional_data?.fact_date_of_lock_ts).format('DD.MM.yyyy • HH:mm')
              }
              readOnly
            />
          </Grid>
          <Grid item xs={12}>
            <StyledInput
              ...
              value={IS_EDITABLE ? data.comment : currentData?.additional_data?.comment}
              onChange={handleOnChange('comment')}
              error={data.comment?.trim().length > 500 ? t('VERIFY_MSG.MAX_500_SYMBOLS') : null}
              readOnly={!IS_EDITABLE}
            />
          </Grid>
          {currentData?.additional_data?.ap_type === 'ZV' && (
            <Grid item xs={12} md={6} lg={3}>
              {IS_EDITABLE ? (
                <SelectField
                  label={t('FIELDS.SEND_TO_MMS')}
                  value={data.send_to_mms}
                  data={[
                    { value: '1', label: t('CONTROLS.YES') },
                    { value: '0', label: t('CONTROLS.NO') }
                  ]}
                  onChange={handleOnChange('send_to_mms')}
                  dataMarker={'send_to_mms'}
                  ignoreI18
                />
              ) : (
                <StyledInput
                  label={t('FIELDS.SEND_TO_MMS')}
                  value={currentData?.additional_data?.send_to_mms ? t('CONTROLS.YES') : t('CONTROLS.NO')}
                  readOnly
                />
              )}
            </Grid>
