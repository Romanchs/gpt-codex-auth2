...
import { useUpdateRequestCorrectionTSMutation, useRequestCorrectionTSReasonsQuery } from '../api';
...
const RejectModal = ({ text = '', open = false, onClose, onSubmit }) => {
  ...
  const [, { error }] = useUpdateRequestCorrectionTSMutation({
    fixedCacheKey: 'requestCorrectionTS_update'
  });
  const { currentData: reasons = [] } = useRequestCorrectionTSReasonsQuery();
