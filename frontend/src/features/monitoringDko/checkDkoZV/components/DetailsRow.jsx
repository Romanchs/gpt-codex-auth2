...
import { useLazyDownloadFileMDCHECKDKOZVQuery, useDetailsMDCHECKDKOZVQuery } from '../api';
...
const DetailsRow = ({ data, columns, isSelect, handleSelect }) => {
  ...
  const { refetch } = useDetailsMDCHECKDKOZVQuery({ uid, params });
  const [downloadFile] = useLazyDownloadFileMDCHECKDKOZVQuery();
  ...
  const getData = (id, value) => {
    if (id === 'created_at' || id === 'finished_at')
      return value && moment(value, 'DD.MM.YYYY HH:mm').format('DD.MM.yyyy • HH:mm');
  ...
  return (
    <TableRow sx={styles.row} data-marker={'table-row'}>
      ...
      {columns.map(({ id, sx }) => {
        if (id === 'is_sent' && !checkPermissions('MONITORING_DKO.DETAILS.TABLE_CELLS.SEND', ['АКО_Процеси'])) return;
        return (
          <TableCell
            key={id + data.uid}
            sx={{
