...
export const getDataForCreate = (filters) => ({
  ap_group: filters.group,
  ...
  metering_grid_areas: Array.isArray(filters.metering_grid_areas)
    ...
    : filters.metering_grid_areas ? [filters.metering_grid_areas] : [],
  checks: filters.checks ? filters.checks?.map((i) => i.value) : null
});
