...
import { getReceivingDkoHistorySubprocess } from '../../../../actions/processesActions';
...
const columns = [
  ...
  { id: 'link', label: 'FIELDS.LINK', minWidth: 200, canDisplay: () => false, align: 'center' }
];
...
const RequestsTab = () => {
  ...
  useEffect(() => {
    dispatch(getReceivingDkoHistorySubprocess(uid, params));
  }, [dispatch, uid, params]);
          ...
          <TableRow>
            {columns.map(({ id, label, minWidth, canDisplay, align }) => {
              return canDisplay() ? (
                ...
                checkPermissions('PROCESSES.RECEIVING_DKO_HISTORICAL.MAIN.TABS.REQUESTS.TABLE_CELLS.LINK', [
                  'АКО',
                  'АКО_Процеси',
                  'АКО_Користувачі',
                  'АКО_Довідники'
                  ...
                  {checkPermissions('PROCESSES.RECEIVING_DKO_HISTORICAL.MAIN.TABS.REQUESTS.TABLE_CELLS.LINK', [
                    ...
                    <TableCell data-marker={'link'} align={'center'}>
                      <CircleButton
                        type={'link'}
                        size={'small'}
                        onClick={() => navigate(`/processes/pon/request-historical-dko/${row?.uid}`)}
