...
import { getReceivingDkoActual } from '../../../../actions/processesActions';
...
const DetailsTab = () => {
  ...
  useEffect(() => {
    dispatch(getReceivingDkoActual(uid, params));
  }, [dispatch, uid, params]);
      ...
      <div className={'boxShadow'} style={{ padding: 24, marginTop: 16, marginBottom: 16 }}>
        <Grid container spacing={3} alignItems={'flex-start'}>
          ...
          <Grid item xs={12} md={6} lg={3}>
            <StyledInput
              label={
                currentProcess?.status === 'CANCELED_BY_OWNER' ? t('FIELDS.CANCELED_AT') : t('FIELDS.COMPLETE_DATETIME')
              }
              readOnly
              value={
          ...
          <Grid item xs={12} md={6} lg={3}>
            <StyledInput
              ...
              readOnly
              value={currentProcess?.dko_data && moment(currentProcess?.dko_data).format('DD.MM.yyyy')}
            />
          </Grid>
          <Grid item xs={12} md={6} lg={3}>
            <StyledInput
