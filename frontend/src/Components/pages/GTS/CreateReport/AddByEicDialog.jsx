...
import { gtsUpdateReportByEic, gtsVerifyTko } from '../../../../actions/gtsActions';
...
const AddByEicDialog = ({ open, onClose }) => {
  ...
  const handleOnClose = () => {
    ...
    if (!reportByEic?.tkos?.data?.length) {
  ...
  const handleAdd = () => {
    dispatch(gtsUpdateReportByEic(type === ADD_ONE ? { include_tko: verifyState.data } : { selected_all_tko: true }));
