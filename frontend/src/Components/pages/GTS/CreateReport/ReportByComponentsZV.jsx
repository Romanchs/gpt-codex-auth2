...
const ReportByComponentsZV = () => {
  const { t } = useTranslation();
  const dispatch = useDispatch();
  const { reportByComponentsZV, loading } = useSelector(({ gts }) => gts);

  const list = useMemo(() => reportByComponentsZV?.tkos?.data || [], [reportByComponentsZV]);
  const currentParams = useMemo(
    () => ({
      page: reportByComponentsZV?.tkos?.current_page || 1,
      size: reportByComponentsZV?.tkos?.size || 25
    }),
    [reportByComponentsZV]
  );

  useEffect(() => {
    dispatch(generalSettingsReport(REPORT_TYPE.BY_COMPONENTS_ZV));
  }, [dispatch]);

  const onDelete = (uid) => () => {
    dispatch(gtsUpdateReportByComponentsZV({ exclude_tko: uid }));
  };

  const onPaginate = (params) => {
    dispatch(gtsGetReportByComponentsZV({ ...currentParams, ...params }));
  };
...
export default ReportByComponentsZV;
